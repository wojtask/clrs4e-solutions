...
EOF
  ...
  mv "${name}.png" light/
  sed 's/\\infile/-channel RGB -negate \\infile/' "${name}.tex" > "${name}_dark.tex"
  pdflatex -halt-on-error -interaction=batchmode -shell-escape "${name}_dark.tex"
  mv "${name}_dark.png" dark/"${name%_dark.png}.png"
  rm -f "${name}".{tex,aux,log,pdf} "${name}_dark".{tex,aux,log,pdf}
done
